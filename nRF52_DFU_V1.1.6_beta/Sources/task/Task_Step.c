...
Sport_Type last_sport_type = SPORT_TYPE_NONE;
unsigned char last_flag = 0;

unsigned long last_cunt_rope_skipping = 0;
unsigned long last_cunt_press_ups = 0;
unsigned char ups_enable = 0;

unsigned long taskStep( unsigned char task_id, unsigned long events )
{
	static unsigned long stepStatusMachine = 0, stepInterruptTime = 0;
    unsigned char local_tx_buf[20];
	...
	{ 
		...
		if ( (d4time < (4-1)) || (d4time > (4+1)) ) /* 4*system tick(50ms) = 200ms */
		{
		...
		for ( unsigned int i = 0; i < FIFO_DEPTH; i++ )
		{
	      ...
	      /* switch to  2G range */	
		if ( range != BMA2x2_RANGE_2G )
		{
			range = BMA2x2_RANGE_2G;				
			osal_set_event ( task_id, TASK_STEP_BMA_RE_INIT_EVT );
		}	
		...
		osal_start_timerEx( task_id, TASK_STEP_BMA_RE_INIT_EVT, 3000 ); 
		
		return ( events ^ STEP_TASK_DECT_EVT );
	}
	
	if ( events & TASK_STEP_UPS_ENABLE_EVT )
	{
		ups_enable = 1;
		
		last_cunt_press_ups = accelDevice.allCounter;
		
		return ( events ^ TASK_STEP_UPS_ENABLE_EVT );
	}
	...
	{ 
		...
		{ 
			{
				sum_x = accel_fifo[i].x;
				sum_z = accel_fifo[i].z;
				sum_y = accel_fifo[i].y;
			}
...
//			if ( (sum_y < -230) || (sum_x > 200) )  /* Turn over the wrist */
			if ( (sum_y > 50) || (sum_x > 20) )  /* Turn over the wrist */
			{
		...
		if ( config.control & BSTOTW_FLAG_TEMP_ENABLE )
		{
			 ...
			 && (config.control & BSTOTW_FLAG_TURN_OVER_ENABLE) )
			{
				...
				if ( (win_current == WIN_ID_TAKE_PHOTO) ||(win_current == WIN_ID_NOTIFY) || (sport_enable == 1))
				{
				 ...
				 * 修改该参数可以调整到达翻腕灭屏效果.
				 */
				osal_start_timerEx( task_id, TASK_STEP_LCD_OFF_EVT, 500);
    ...
    if ( events & TASK_STEP_LCD_OFF_EVT )
    {
        config.control &= ~BSTOTW_FLAG_CC_ENABLE;
        
        return ( events ^ TASK_STEP_LCD_OFF_EVT );
    }
    ...
    {
		static short xo1 = 0,xo2 = 0,xo3 = 0;
		static short abs_x = 0,abs_y = 0, abs_z = 0;
        
		for ( int i = 0; i < FIFO_DEPTH; i += 2 )
		{
    ...
    ***************************************************************************/
    ...
    if ( events & STEP_TASK_NOTIFY_SLEEP_TEST_EVT )
    {
        ...
        */
        ...
        local_tx_buf[2] = AutoSleep.sleep_step;
        local_tx_buf[3] = 0x55;
        
        local_tx_buf[4] = AutoSleep.wave_cnt;
        local_tx_buf[5] = 0x55;
        
        local_tx_buf[6] = (AutoSleep.action_cnt)>>8;
        local_tx_buf[7] = (AutoSleep.action_cnt);
        local_tx_buf[8] = 0x55;
        ...
        bt_protocol_tx(local_tx_buf, 20);
	...
	if ( events & TASK_STEP_UPLOAD_PEDOMETER_EVT )
	{
        ...
        {       
            ...
            if ( (accelDevice.allCounter - accelDevice.oCounter) > 9)
            {
                stepStatusMachine = 1;              
	...
	{
		if ( (taskStepInterrutCount > (8-4)) && (taskStepInterrutCount < (8+4)) )
		{
...
void taskStepInit( unsigned char task_id )
{
	taskStepTaskId = task_id;
	
	heart_comp_init ( config.age, config.height, config.weight );
	step_algorithm_mode_set(ALGORITHM_PEDOMETER);
	Alogrithm_Init();
