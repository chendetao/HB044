...
unsigned long taskTouch( unsigned char task_id, unsigned long events )
{
	...
	if ( events & TASK_TOUCH_DECTING_EVT )
	{
		if ( nrf_gpio_pin_read(BUTTON_PIN) == 0 )
		{
			...
			if ( counter > 24 )
			{
				if(win_current == WIN_ID_POWER)                   
				{	
					if ( (config.shutdown == 0) && (config.lcdEnable == 0) )
					{
...
void taskTouchInit( unsigned char task_id )
{
