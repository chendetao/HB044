...
#define DEVICE_NAME                      "A8"                                       /**< Name of device. Will be included in the advertising data. */
...
unsigned long taskBle( unsigned char task_id, unsigned long events )
{
	...
	if ( events & TASK_BLE_CONNECTED_EVT )
	{
        ...
        osal_start_timerEx ( task_id, TASK_BLE_SYNC_COMFIRM_EVT, 5000 );
        
		/** ǁĻϨİ״̬݆ܲ֟ҽާĦДʾ׾άëʱࠉӔ TOUCH */
	...
	if ( events & ANCS_DISCOVERY_DELAY_START_EVT )
	{
		...
		ble_db_discovery_start(&m_ble_db_discovery, m_peer_handle.connection_id);
		
		return ( events ^ ANCS_DISCOVERY_DELAY_START_EVT );
	}	
    
    if ( events & TASK_BLE_SYNC_COMFIRM_EVT )
    {
        if ( (ble.isConnected == 1) && (sync_flag == 0) )
        {
//            ConnectionTerminate(1);
        }
        
        return ( events ^ TASK_BLE_SYNC_COMFIRM_EVT );
    }
...
void taskBleInit( unsigned char task_id )
{
