...
Algorithm_mode algorithm_mode = ALGORITHM_PEDOMETER;
...
int maxx = 0;
 ...
 int step_algorithm_mode_set( Algorithm_mode mode )
{
	if ( mode == ALGORITHM_PEDOMETER )
	{
		maxx = ALGORITHM_THRESHOLD_DEFAULT;
	}else{
		maxx = ALGORITHM_THRESHOLD_MAX;
	}
...
{    
    Step_main_thread(&accel_fifo[0]);
    Step_main_thread(&accel_fifo[1]);
    Step_main_thread(&accel_fifo[2]);
    Step_main_thread(&accel_fifo[3]);
    Step_main_thread(&accel_fifo[4]);
    Step_main_thread(&accel_fifo[5]);
    Step_main_thread(&accel_fifo[6]);
    Step_main_thread(&accel_fifo[7]);    
}
