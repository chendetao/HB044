...
#if 0
const int lcd_level[] = {100,120,140,160,180,200,220,240,260,280,300,320,340,360,380,400,420,440,460,480,500,520,540,560,580,600,700,800,900,990};
#else
const short lcd_level[] = {500,700,990};    
#endif
...
int bt_protocol_rx(const unsigned char *buf, int length )
{
	...
	case 0x0F: /* Firmware maintenance */
			...
			{
				/** Set Lcd level */
				config.lcd_level = lcd_level[buf[3]%(sizeof(lcd_level)/sizeof(lcd_level[0]))];
			...
			{
				hr.touch_level = ( (buf[3]<<8)|buf[4] ) & 0xFFF;
				txbuf[1] = 0x14;
			}else if ( buf[2] == 0x16 ) /** power off */		
			{
				osal_start_timerEx( taskGUITaskId, TASK_GUI_SYS_POWEROFF_EVT,500);
				txbuf[1] = 0x16;
			}else if ( buf[2] == 0x18) /* Set G-Sensor measure range */
			{
				txbuf[1] = 0x18;
				range = buf[3];
				
				osal_set_event ( taskStepTaskId, TASK_STEP_BMA_RE_INIT_EVT );
			}else if ( buf[2] == 0x1A ){
				osal_set_event ( taskFlashTaskId, TASK_FLASH_RESET_EVT );
				return 0;
			}else if ( buf[2] == 0x30 ){
				/** Set Lcd level */
                unsigned short temp = ((buf[3]<<8) | buf[4]) % 1000;
				config.lcd_level = temp;
				osal_set_event ( taskGUITaskId, TASK_GUI_LCD_LEVEL_UPDATE_EVT);                
            }else{
				txbuf[0] |= 0x1<<7;
			}
	...
	case 0x22: /* Automatic heart rate measurement configuration */
			...
			break;	
	case 0x23: // Getting sleep statistics
			if ( buf[2] == 0x1 )
			{
				osal_set_event( taskStepTaskId, TASK_STEP_SLEEP_STATISTICS_EVT);
				return 0;
			}else{
				txbuf[0] |= 0x1<<7;
			}break;	            
