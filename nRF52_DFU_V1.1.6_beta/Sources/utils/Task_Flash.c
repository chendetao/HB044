...
int flash_memory_Init(void)
{
	...
	if ( ((struct FlashMemory*)swap)->control_magic != 0x8A7B43C2 )
	{
		/**
		 * TODO : 在此将所有需要的设置全部设置为默认状态.
		 */
		fm.control_magic = 0x8A7B43C2;		
	...
	{		
		fm.test_mode &= 0x1;	//make the number [test_mode] value in range of [0,1]		
...
unsigned long taskFlash( unsigned char task_id, unsigned long events )
{ 
    unsigned char action_arr[20] = {0x0F};
	...
	if ( events & TASK_FLASH_PUT_BP_EVT )
	{
        ...
        if ( put_cnt++ < 128 )
        {
            ...
            buf[6] = 98;
            buf[7] = osal_systemClock % 10;
            
            flash_temp_queue_put(FM_ID_SPO2, buf,16);
            
            osal_start_timerEx ( task_id, TASK_FLASH_PUT_BP_EVT, 800 );		
        }else{
            put_cnt = 0;
        }
...
void taskFlashInit( unsigned char task_id )
{
