...
#define BMA_POWER_DOWN()  nrf_gpio_pin_set(BMA_POWER_PIN)
#define BMA_POWER_ON()    nrf_gpio_pin_clear(BMA_POWER_PIN)
...
unsigned char range = BMA2x2_RANGE_8G;
...
int bma250_init( void )
{
    ...
    }while( BackRead != BMA2x2_MODE_NORMAL && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != BMA2x2_BW_500HZ && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != ACTIVE_HIGH && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != BMA2x2_LATCH_DURN_1MS && (++cnt<TRY_CNT)); 
    if( cnt >= TRY_CNT )
      error_cnt++;

    cnt = 0;
    do{
        bma2x2_set_range(range);
        bma2x2_get_range( &BackRead );
    }while( BackRead != range && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != 0x0 && (++cnt<TRY_CNT)); 
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != FIFO_DEPTH && (++cnt<TRY_CNT)); 
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != INTR_ENABLE && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    }while( BackRead != BMA2x2_SLEEP_DURN_25MS && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
    ...
    do { 
        ...
        bma2x2_get_power_mode( &BackRead );
    }while( BackRead != BMA2x2_MODE_LOWPOWER1 && (++cnt<TRY_CNT));
    if( cnt >= TRY_CNT )
      error_cnt++;
